...
class Dao(object):
    ...
    def find_all_date(self): #only called for ordering by date
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY date ASC'.format(self._table_name))
        return orm(c, self._dto_type)
    def find_all_Primary(self):#called for ordering by the primary key
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY id ASC'.format(self._table_name))
        return orm(c, self._dto_type)
    def update(self,id, data):
        cursor = self._conn.cursor()
        set_stmt = ','.join(['{}=?'.format(k) for k in data.keys()])
        stmt = 'UPDATE {} SET {} WHERE id=?'.format(self._table_name, set_stmt)
        cursor.execute(stmt, (*data.values(), id))
